...
router.get("/registered-users", auth_1.authenticate, auth_1.requireAdmin, async (req, res) => {
    try {
        ...
        try {
            const rawRes = await db_1.pool.query('SELECT email, machine_id FROM users WHERE machine_id IS NOT NULL LIMIT 5');
        ...
        const responseUsers = registeredUsers.map((entry) => {
            ...
            const mid = entry.machineId || entry.machine_id;
...
router.get("/transaction-summary", auth_1.authenticate, auth_1.requireAdmin, async (_req, res) => {
    try {
        const rawSummary = await db_1.db
            ...
            lastTransactionAt: (0, drizzle_orm_1.sql) `max(${schema_1.transactions.createdAt})`,
        })
            .from(schema_1.users)
            .leftJoin(schema_1.transactions, (0, drizzle_orm_1.eq)(schema_1.users.id, schema_1.transactions.userId))
            .groupBy(schema_1.users.id, schema_1.users.email, schema_1.users.planType, schema_1.users.subscriptionDuration, schema_1.users.accountStatus, schema_1.users.renewalDate, schema_1.users.paymentFinalAmount, schema_1.users.createdAt, schema_1.users.name, schema_1.users.companyName, schema_1.users.mobile, schema_1.users.couponDiscountAmount);
